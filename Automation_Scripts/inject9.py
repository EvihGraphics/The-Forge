...
replace_block(r'void AdaptiveVoxelBasedOITPass\(Cmd\* pCmd\)\s*\{\s*UNREF_PARAM\(pCmd\);\s*// Placeholder for AVBOIT Clear, Build, Resolve\s*\}', '''void AdaptiveVoxelBasedOITPass(Cmd* pCmd)
{
    ...
    for (uint32_t i = 0; i < gTransparentDrawCallCount; ++i)
    {
        ...
        if (dc->mDrawIdx >= gDataBufferCount) continue;

...
RES(RWTex3D(uint), VolumeExtinctionBufferUAV, UPDATE_FREQ_PER_FRAME, u1);
RES(RWTex3D(float4), VolumeTransmittanceLutUAV, UPDATE_FREQ_PER_FRAME, u2);
RES(RWTex3D(uint), VolumeColorBufferUAV, UPDATE_FREQ_PER_FRAME, u3);
#endif

#ifdef AVBOIT_ORDERED_ACCESS
RES(Tex3D(uint), VolumeExtinctionBufferSRV, UPDATE_FREQ_NONE, t1);
RES(Tex3D(float4), VolumeTransmittanceLutSRV, UPDATE_FREQ_NONE, t2);
RES(Tex3D(uint), VolumeColorBufferSRV, UPDATE_FREQ_NONE, t3);
#endif

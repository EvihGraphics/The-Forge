...
function Get-CaptureWindowHandle([IntPtr]$candidate) {
    if ($candidate -ne [IntPtr]::Zero) {
        $probeRect = New-Object ForgeModeCaptureWin32+RECT
        if ([ForgeModeCaptureWin32]::GetClientRect($candidate, [ref]$probeRect)) {
            return $candidate
        }
    }

    return [ForgeModeCaptureWin32]::FindWindow($null, "15_Transparency")
}
...
function Get-ClientBounds([IntPtr]$handle) {
    ...
    if (-not [ForgeModeCaptureWin32]::GetClientRect($handle, [ref]$rect)) {
        return $null
    }

    $point = New-Object ForgeModeCaptureWin32+POINT
    $point.X = 0
    $point.Y = 0
    if (-not [ForgeModeCaptureWin32]::ClientToScreen($handle, [ref]$point)) {
        return $null
    }
        ...
        if ($bounds -ne $null) {
            ...
            if (-not $p.HasExited -and $p.MainWindowHandle -ne 0) {
                ...
                if ($bounds -ne $null) {
                    ...
                    $hdc = $g.GetHdc()
                    try {
                        $printed = [ForgeModeCaptureWin32]::PrintWindow($activeHandle, $hdc, 2)
                    } finally {
                        $g.ReleaseHdc($hdc)
                    }

                    if ($printed) {
                        $captureStatus = "CAPTURED_CLIENT_PRINTWINDOW"
                    } else {
                        $g.CopyFromScreen($bounds.Left, $bounds.Top, 0, 0, (New-Object System.Drawing.Size($width, $height)))
                        $captureStatus = "CAPTURED_CLIENT_COPYFROMSCREEN"
                    }
                    ...
                    $captureError = "GetClientRect/ClientToScreen failed after mode selection"
                }
            } else {
            ...
            $captureStatus = "CAPTURE_FAILED"
            $captureError = "Initial GetClientRect/ClientToScreen failed"
        }
    }
}
catch {
